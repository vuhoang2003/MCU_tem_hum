...
#define MAX_USB_REC_BUFF 255
#define TOGGLE_LED_REC  		GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, (GPIO_PIN_3^GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_3)))
//*****************************************************************************
//
...
static bool g_Sen_Get_Data = false;
unsigned char I2C_Sen_Data[10];
char Mybuff[80];
...
unsigned int Usb_Write_Num=0;
unsigned int Usb_Read_Num=0;
unsigned char Usb_Rec_Buff[MAX_USB_REC_BUFF]={0};
char L_Receive_Buf[100]={0};
unsigned char CMD_REQ=0;


static void UIUsbcanReceive(void)
{
    unsigned int ucSize;
    unsigned long ulIdx;
    ...
    //
    while(Usb_Read_Num != Usb_Write_Num)
    {
        //
        // See if this character is the tag for the start of a command packet.
        //
        if(!((Usb_Rec_Buff[Usb_Read_Num] == 'R')&&
				(Usb_Rec_Buff[(Usb_Read_Num+1)%MAX_USB_REC_BUFF] == 'D')))
        {
            //
            // Skip this character.
            //
            Usb_Read_Num = (Usb_Read_Num + 1) % MAX_USB_REC_BUFF;
        ...
        //
        if(((Usb_Read_Num + 2) % MAX_USB_REC_BUFF) == Usb_Write_Num)
        {
            //
        ...
        //
        ucSize = Usb_Rec_Buff[(Usb_Read_Num + 2) %  MAX_USB_REC_BUFF]-0x30;// length of pack from 5 to 9
        if((ucSize < 4) || (ucSize > (MAX_USB_REC_BUFF - 1)))
        {
            //
            ...
            //
            Usb_Read_Num =
                (Usb_Read_Num + 2) % MAX_USB_REC_BUFF;
        ...
        //
        ulIdx = Usb_Write_Num - Usb_Read_Num;
        if(Usb_Read_Num > Usb_Write_Num)
        {
            ulIdx += MAX_USB_REC_BUFF;
        }
		...
		for(ulIdx = 0; ulIdx < ucSize; ulIdx++)
		{
			L_Receive_Buf[ulIdx] = Usb_Rec_Buff[(Usb_Read_Num + ulIdx) % MAX_USB_REC_BUFF];
		}
		if (strncmp (&L_Receive_Buf[3],"TnH",ucSize-3) == 0)
		{
			CMD_REQ = RD_T_H;
			USBBufferWrite((tUSBBuffer *)&g_sTxBuffer, (unsigned char *)Mybuff, count_str);
		}
		else if (strncmp (&L_Receive_Buf[3],"RPY",ucSize-3) == 0)
		{
			CMD_REQ = RD_RPY;
		}
		else if (strncmp (&L_Receive_Buf[3],"CO2",ucSize-3) == 0)
		{
			CMD_REQ = RD_RPY;
		}
		else
		{	
			Usb_Read_Num = (Usb_Read_Num + 2) % MAX_USB_REC_BUFF;
			// Keep scanning for a start of command packet tag.
			continue;				
		}
		TOGGLE_LED_REC;		
        ...
        //
        Usb_Read_Num = (Usb_Read_Num + ucSize) % MAX_USB_REC_BUFF;
    }
}
...
void CMD_Process(void)
{
	...
	switch (CMD_REQ)
	{
		case RD_T_H:
		{			
			USBBufferWrite((tUSBBuffer *)&g_sTxBuffer, "Hello", 5);
			CMD_REQ = 0;
			break;
		}
		case RD_RPY:
...
SysTickIntHandler(void)
{
    ...
    //
    g_ui32SysTickCount++;
	if((g_ui32SysTickCount%30)==0) g_Sen_Get_Data = true;
}
          ...
          void *pvMsgData)
{
    ...
    switch(ui32Event)
    {
        ...
        case USB_EVENT_RX_AVAILABLE:
        {
			...
			{
				// Read a character
				Usb_Rec_Buff[Usb_Write_Num] = ui8Char;
				Usb_Write_Num = (Usb_Write_Num + 1) % MAX_USB_REC_BUFF;
				if((Usb_Write_Num+1)%MAX_USB_REC_BUFF == Usb_Read_Num)
				{
					UIUsbcanReceive();
				}
...
main(void)
{
    ...
    while(1)
    {
		...
		if(g_Sen_Get_Data)
		{
			g_Sen_Get_Data = false;
			SHT_ReadTemHumi(&nhiet_do,&do_am);
			count_str=sprintf( Mybuff, "Temperature: %d.%d°C. Humidity: %d.%d .\n", (int)nhiet_do,  (int)(((nhiet_do - (float)((int)nhiet_do))) * 10), (int)do_am,  (int)(((do_am - (float)((int)do_am))) * 10));
			Mybuff[count_str-3]='%';
		}
    }
}
